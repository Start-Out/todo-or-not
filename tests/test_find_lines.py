...
class TestFindLines(unittest.TestCase):
    ...
    def test_find_lines(self):  # TODO this test is super flakey, it might just need to be split out into several. Definitely needs expected and eq check now that that's a thing
        ...
        expected_hits_by_name[formatted_todo_in_pretty_function.__repr__()] = formatted_todo_in_pretty_function

        todo_in_closed_function = Hit(self.hit_tests, 41, ["todo"], ['def a_closed_example():\n', '    # TODO Closed Issues are helpful! | This issue is closed, but the TODO string is still in the codebase!\n', '    print("This should be a red flag, because if the issue is still in the code then something isn\'t done yet")\n', '    print("(Though it may simply be that the comment hasn\'t been removed)")\n'], 1)
        todo_in_closed_function.structured_title = "TODO Closed Issues are helpful!"
        todo_in_closed_function.structured_body = "This issue is closed, but the TODO string is still in the codebase!"
        expected_hits_by_name[todo_in_closed_function.__repr__()] = todo_in_closed_function
